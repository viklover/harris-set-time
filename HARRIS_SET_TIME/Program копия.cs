...
namespace HARRIS_SET_TIME
{
    class Program
    {
        ...
        static bool develope_mode = true;

        static bool ready()
        {
            if (DateTime.Now.Subtract(last_send).Seconds > 2.0 &&
                ...
                foreach (string update in GetUpdates())
                {
                    switch (update)
                    {
                        case "input":
                            ...
                            byte[] time_set = Encoding.ASCII.GetBytes($"set time {DateTime.Now.ToString("HH:mm:ss")}").Concat(GetBytes("0D")).ToArray();
        ...
        {
            //string result = System.Text.Encoding.ASCII.GetString(message);
